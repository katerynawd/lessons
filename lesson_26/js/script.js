...
if(entry.isIntersecting){
	//відкривається цільовий елемент та отримуються дата-атрибути
	...
	const interval = parseInt(itemElement.getAttribute('data-interval'));

	//якщо елемент не видноб то він не спостирігається
	observer.unobserve(itemElement);
	let counter = startNumber;
	// константа, яка відповідає за додавання інтервалу
const intervalId = setInterval(()=>{
 ...
 let observer = new IntersectionObserver(updateContent, options);
 const item = document.querySelector('.item-main');
 if (item){
